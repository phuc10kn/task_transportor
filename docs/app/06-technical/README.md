...
- Auth dùng Bearer JWT principal `user` với `POST /api/v1/auth/login`, `POST /api/v1/auth/google`, `POST /api/v1/auth/logout`, `GET /api/v1/auth/me`. Google cần `GOOGLE_LOGIN_ENABLED`, `GOOGLE_CLIENT_ID`, `PUBLIC_ORIGIN`, chỉ map verified email sang user enabled có sẵn và không lưu ID token.
- System-admin user API là `GET/POST /api/v1/users` và `PATCH /api/v1/users/:userId`. Project Team API là `GET /api/v1/projects/:projectId/team`, `POST .../team/members`, `PATCH/DELETE .../team/members/:userId`.
...
- Project middleware kiểm tra membership rồi trả `404 PROJECT_NOT_FOUND`, `409 PROJECT_DISABLED` hoặc `422 PROJECT_SCOPE_MISMATCH`; owner lookup sai Project trả `404 RESOURCE_NOT_FOUND`. System role không bypass membership. Route workspace global cũ không còn được mount.
