...
- Operational trace phải nối được HTTP → enqueue → worker → external call bằng trace/correlation bền; từng provider có request/response riêng, body JSON/text sau redaction, binary omission và network error rõ. Log không được chứa canary credential và không thay thế `sync_journal`.
...
- Password và Google login phải trả cùng safe user shape. Mọi authenticated user tự sửa được display name nhưng không thể đổi email/system role qua self-profile API; name được trim, giới hạn 120 ký tự và lỗi UI giữ input. Google-first tạo user role `user`; password-first phải link explicit tại `My account`. Google từ chối origin sai, token không hợp lệ, email chưa verify, identity conflict, email link mismatch hoặc user disabled. Chỉ `system_admin` quản lý/hard-delete user; không được hạ system admin enabled cuối cùng, tự xóa, xóa system admin enabled cuối cùng hoặc làm mất Project owner. Sau hard-delete, identity phụ thuộc bị dọn và JWT cũ không còn authenticate được.
- Project create phải tạo dedicated Team, creator owner + lead trong cùng transaction. User ngoài Team (kể cả `system_admin`) không thấy Project; member không sửa config; lead quản lý member; owner không thể bị xóa/hạ role; xóa Project dọn Team/membership atomically. `system_admin` được chuyển ownership qua control-plane: owner mới là lead, owner cũ là member trong cùng transaction; actor admin ngoài Team vẫn nhận `404` từ workspace và không xuất hiện trong membership sau chuyển quyền.
...
- `npm run verify:observability`: schema/event order, durable trace, provider routing, body/redaction và error/binary contract.
