...
- Login giữ email/password và hiển thị Google Identity button khi server bật Google login. Cả hai đường đăng nhập trả cùng user session `cis_user_token`; Google không có luồng đăng ký.
- Route toàn cục `/users` chỉ hiện cho `system_admin`, hỗ trợ tạo user và đổi `system_role`. Project Config hiển thị access badge và Team member list; owner sửa config, lead thêm user enabled bằng email chính xác/đổi role/xóa member, member chỉ xem.
...
- `Login`, `Users` và `Projects` là route toàn cục (`/login`, `/users`, `/projects`). Dashboard, Mappings, Backlog Issues, CIS Issues/Issue Editor, Translation Queue/Glossary, Anomalies, Sync Jobs và Journal chỉ tồn tại dưới `/project/:projectId/...`; không duy trì route workspace toàn cục cũ hoặc `project_id` query fallback.
