...
- `Pull project` luôn hiển thị disabled với lý do rõ; operator dùng `Sync to CIS`, `Sync + Translate` hoặc `Sync + Translate + Jira` theo từng candidate.
...
- Màn CIS Issues có form tạo issue thủ công và Issue register phân trang cố định 20 dòng. Search chỉ contains không phân biệt hoa thường theo canonical Summary. Priority và Assignee không nằm trong khu vực search; hai cột này vẫn hiển thị giá trị CIS, không dùng source Backlog/Jira. Issue Editor cho link Backlog issue/Jira task khi field còn trống.
