...
- Login giữ form email/password và hiển thị Google Identity button khi deployment bật Google. Google-first tự tạo CIS user role `user`; cả hai đường trả cùng session `cis_user_token`. Google lỗi không làm mất input hoặc chặn password login.
- Route toàn cục `/account` mở từ email trên toolbar cho mọi user, có Profile settings để tự sửa display name và hiển thị email identity read-only; bên dưới hiển thị hai sign-in method của chính user: password-first có thể link Google cùng verified email, Google-first có thể đặt password một lần. Lỗi save giữ input và thành công cập nhật tại chỗ. Route `/users` chỉ hiện cho `system_admin`, quản lý danh sách CIS user, tạo user, đổi `system_role`, hard-delete user sau confirm và có card `Project ownership`. Card này cho chọn user enabled làm owner mới, confirm rõ Current owner → New owner, giải thích owner mới thành Team lead/owner cũ thành member và chỉ cập nhật hàng Project sau khi thành công, không reload document hay danh sách user. Nút xóa chính mình bị khóa, Project owner chưa xử lý ownership bị Backend chặn. Route này không chứa Google policy hoặc deployment config. Project Config hiển thị access badge và Team member list; owner sửa config, lead thêm user enabled bằng email chính xác/đổi role/xóa member, member chỉ xem.
...
- `Login`, `My account`, `Users` và `Projects` là route toàn cục (`/login`, `/account`, `/users`, `/projects`). Dashboard, Mappings, Backlog Issues, CIS Issues/Issue Editor, Translation Queue/Glossary, Anomalies, Sync Jobs và Journal chỉ tồn tại dưới `/project/:projectId/...`; không duy trì route workspace toàn cục cũ hoặc `project_id` query fallback.
