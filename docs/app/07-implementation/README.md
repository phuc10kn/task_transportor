...
- Auth runtime dùng `UserRepository`, `user_identities`, user JWT và Google ID-token verifier tại external provider boundary; Admin Web tách Login, `My account` sign-in methods và system-admin Users. Projects tạo Project + Team + owner lead atomically, lọc Project list theo membership và dùng cùng membership guard cho mọi workspace route.
