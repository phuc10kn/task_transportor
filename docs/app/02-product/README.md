...
- Người dùng đăng nhập bằng email/password hoặc Google và tự sửa display name tại `My account`; email identity là read-only vì dùng cho đăng nhập/Google matching. Google-first tự tạo CIS user role `user`; password-first link Google explicit tại `My account`, không auto-link chỉ vì trùng email. Google chỉ là sign-in identity, không cấp system role hoặc quyền Project. `system_admin` quản lý danh sách user, system role, hard-delete user và có control-plane riêng để chuyển Project owner sang một user enabled; owner mới thành Team lead, owner cũ ở lại Team với role member. System admin không được tự xóa, xóa system admin enabled cuối cùng hoặc xóa Project owner khi ownership chưa được xử lý; quyền chuyển owner không tự thêm system admin vào Team hoặc cấp quyền workspace.
