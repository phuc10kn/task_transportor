...
## Canonical Graph Slice Hiện Tại

Outbound Jira safety slice đã materialize relation canonical cho dry-run và sync thật:

- AF-006 và AF-007 dùng `involves` tới các module thực sự tham gia.
- MB-006 dùng `constrains` tới module Jira thực thi outbound write.
- Cis, Mapping, Anomaly và Sync dùng `owns` tới state owner tương ứng.

Slice này phục vụ trace impact của Jira outbound. Nó không materialize toàn bộ prose `Related Entities`, không tạo dual/inverse edge và không tạo relation Boundary-to-Flow khi chưa có valid triple.
