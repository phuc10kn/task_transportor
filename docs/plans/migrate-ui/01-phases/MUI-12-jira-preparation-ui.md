...
- Populate editable Jira fields từ dry-run payload: summary, description, issue_type, priority, status, assignee và due_date. `issue_type`, `priority`, `status` là select lấy option từ `editor.field_meta.catalogs_by_system.jira`; không hard-code hoặc đổi thành free text.
