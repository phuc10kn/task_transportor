...
3. Cập nhật `## Quy ước điều phối > ### Handoff hiện tại` để lượt sau resume đúng điểm.
...
- cập nhật `## Quy ước điều phối > ### Trạng thái blocked` bằng format `Blocked: <phase id> - <blocker ngắn>`;
- dừng và báo lại.
...
```text
Đã triển khai theo plan tại <plan-file>.
