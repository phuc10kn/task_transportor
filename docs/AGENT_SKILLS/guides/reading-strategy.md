...
## Reading priority hints

Một số layer có thể dùng khái niệm `core` / `supporting` để ưu tiên đọc, nhưng đây chỉ là heuristic.
...
Agent có thể đọc core entities trước để định hướng nhanh, nhưng không được bỏ qua supporting entity nếu nó nằm trên valid trace path hoặc task yêu cầu.

Không dùng `core/supporting` như schema canonical trừ khi `docs/meta/` định nghĩa field đó trong tương lai.
