...
docs/workbench/    -> temporary knowledge workspace; CIS scope active theo DEC-003
...
Đây là đường mặc định cho mọi task docs. Các skill đang active trong index bên dưới đọc guide và làm việc với canonical home. Khi gặp undetermined-placement và Workbench đã active, standard agent route sang `workbench-intake` rồi handoff lại standard flow để promote.

### Workbench-Support Agent

Workbench agent hỗ trợ intake/triage/update theo policy local. `task_transportor` đã kích hoạt CIS Workbench theo DEC-003. Workbench-support không approve app/meta/theory truth và phải handoff về standard flow để materialize canonical.
...
- Làm Admin UI: dùng `ui-design` và Playwright theo scope, đồng thời tuân `docs/app/03-interface/README.md`.
...
- Không dùng `docs/workbench` làm source of truth; chỉ dùng theo DEC-003/policy local.
- Standard agent là mặc định; workbench-support chỉ hỗ trợ intake/lifecycle và phải handoff về standard flow để promote.
