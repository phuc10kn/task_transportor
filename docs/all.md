...
> Canonical root hiện tại là `docs/`.
...
`application.md` là cấu trúc cũ. Trong mô hình hiện tại, việc áp dụng theory nằm trong `docs/app/` qua `theory_basis`, không đặt file `application.md` trong từng theory.
...
```text
docs/theories/human-governed-ai-assistance/
```

Purpose:

Định nghĩa vai trò AI propose/analyze
và human final authority trong hệ thống.

Read when:

* designing AI draft/review flows;
* reviewing AI transport-vs-business boundary;
* changing approval path cho reviewed result.
...
Không dùng `project-theory.md` trong mô hình hiện tại. Theory được tách theo theory group dưới `docs/theories/<slug>/`.
...
```text
read docs/theories/README.md trước, rồi chỉ mở các theory group liên quan
```
