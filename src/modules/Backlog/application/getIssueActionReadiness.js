...
const REASON_ORDER = ["PROJECT_PULL_DISABLED", "PROJECT_DISABLED", "BACKLOG_EXTERNAL_READ_DISABLED", "JIRA_EXTERNAL_READ_DISABLED", "JIRA_EXTERNAL_WRITE_DISABLED", "BACKLOG_CONFIG_INCOMPLETE", "BACKLOG_PULL_DISABLED", "PROJECT_SYNC_DISABLED", "SYNC_WORKER_UNAVAILABLE"];
...
function getIssueActionReadiness({ config, projectId }) {
  ...
  const jiraReasons = [...syncReasons, !project.jira_external_read_enabled && "JIRA_EXTERNAL_READ_DISABLED", !project.jira_external_write_enabled && "JIRA_EXTERNAL_WRITE_DISABLED"].filter(Boolean);

  return {
    project_id: project.id,
    actions: {
      browse: { enabled: configReady && project.backlog_external_read_enabled, disabled_reasons: orderedReasons([!project.backlog_external_read_enabled && "BACKLOG_EXTERNAL_READ_DISABLED", !configReady && "BACKLOG_CONFIG_INCOMPLETE"].filter(Boolean)) },
      ...
      sync_translate_jira: writeAction({ enabled: jiraReasons.length === 0, mode: "queued_ready", consumerReady: queueReady, reasons: jiraReasons }),
    },
  };
}
