...
(() => CIS.ready(({ project }) => {
  ...
  async function queuePage() {
    ...
    let items;
    try {
      const query = new URLSearchParams(); if (status) query.set("review_status", status); if (issueId) query.set("issue_id", issueId);
      items = await CIS.projectApi(project.id, `/translation-queue?${query}`);
  ...
  function patchQueueRow(item, row) {
    row.querySelector("[data-queue-status]").innerHTML = `${CIS.badge(item.review_status || "pending")}${item.is_source_stale ? '<div class="text-warning small mt-1">Source changed</div>' : ""}`;
    row.querySelector('[data-action="approve"]').disabled = Boolean(item.is_source_stale || !item.ai_draft || item.review_status === "approved");
  }

  async function queueAction(item, button, row) {
    const action = button.dataset.action;
    if (action === "edit") return editTranslation(item, row);
    const evidence = row.querySelector(".job-evidence"); const label = button.innerHTML; button.disabled = true; button.setAttribute("aria-busy", "true"); button.innerHTML = '<span class="spinner-border spinner-border-sm me-1" aria-hidden="true"></span>Working…';
    try {
      const result = await CIS.projectApi(project.id, `/translation-queue/${item.id}/${action}`, { method: "POST", body: { review_notes: "translation-queue" } });
      Object.assign(item, result.item || result, action === "retranslate" ? { review_status: result.item?.review_status || "pending" } : {});
      patchQueueRow(item, row);
      ...
      else CIS.toast(`Translation ${action}d.`);
    } catch (error) { evidence.innerHTML = `<span class="text-danger">${CIS.escape(error.message)}</span>`; }
    finally { button.removeAttribute("aria-busy"); button.innerHTML = label; button.disabled = action === "approve" && item.review_status === "approved"; }
  }
  ...
  function editTranslation(item, row) {
    ...
    modal.querySelector("form").addEventListener("submit", async (event) => {
      ...
      try { const saved = await CIS.projectApi(project.id, `/translation-queue/${item.id}/draft`, { method: "PUT", body: { draft_text: event.currentTarget.draft_text.value, review_notes: "translation-queue" } }); Object.assign(item, saved, { is_source_stale: false }); patchQueueRow(item, row); modal.close(); CIS.toast("Translation draft saved."); }
