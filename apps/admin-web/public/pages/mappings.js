...
(() => CIS.ready(async ({ project }) => {
  ...
  function groupRows(group, flow) {
    ...
    return `<tbody data-mapping-group="${CIS.attr(id)}"><tr class="mapping-group-row"><th class="mapping-group-cell" colspan="4" scope="rowgroup"><button class="mapping-group-toggle" type="button" data-toggle-mapping-group aria-expanded="${collapsed ? "false" : "true"}"><span class="mapping-group-toggle__icon" aria-hidden="true">⌄</span><span class="mapping-group-title">${CIS.escape(group.label)}${required ? '<span class="mapping-field-required" title="Required for Jira" aria-label="Required for Jira">*</span>' : ""}</span><span class="mapping-group-meta">${group.items.length} values · ${issueCount} issues</span></button></th></tr>${group.items.map((item) => row(item, flow).replace("<tr ", `<tr ${collapsed ? "hidden " : ""}`)).join("")}</tbody>`;
  }
