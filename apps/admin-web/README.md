...
Copy `.env.example` to `.env.local` and set `CIS_API_ORIGIN` to the Express API origin, then run `npm run dev` from this directory or `npm run admin:dev` from the repository root. If Turbopack keeps a stale `globals.css` bundle during local visual work, run `npm run admin:dev:stable`: it clears only `.next/dev` and restarts the local dev server after each `globals.css` save. For a local production smoke, run `npm run build` then `npm run start -- --port 8001`. The browser only uses relative `/api/v1/*`; `next.config.ts` rewrites those requests server-side. The default local value is `http://127.0.0.1:3000`. Express no longer serves the former static UI.
