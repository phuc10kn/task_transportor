...
test("mappings keeps URL context, saves both directions and exposes refresh actions", async ({ page }) => {
  ...
  await page.route("**/api/v1/projects/1/**/mapping-values/**", async (route) => { await new Promise((resolve) => setTimeout(resolve, 150)); return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ data: {} }) }); });
  await page.goto("/login?next=%2Fmappings%3Fproject_id%3D1%26source_system%3Dbacklog%26target_system%3Djira");
  await page.getByLabel("Email").fill("admin@example.com"); await page.getByLabel("Password").fill("12345678"); await page.getByRole("button", { name: "Sign in" }).click(); await page.getByRole("link", { name: "Choose or create Project" }).click(); await page.getByRole("button", { name: "Open workspace" }).first().click();
...
test("mappings exposes empty and retry states", async ({ page }) => {
  ...
  await expect(page.getByText("Workspace unavailable")).toBeVisible(); await page.getByRole("button", { name: "Retry" }).click(); await expect(page.getByRole("heading", { name: "Choose a Project first" })).toBeVisible(); await page.getByRole("link", { name: "Choose or create Project" }).click(); await expect(page.getByText("No projects configured")).toBeVisible();
});
