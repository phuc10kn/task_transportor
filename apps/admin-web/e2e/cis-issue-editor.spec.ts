...
test("CIS Issues editor keeps canonical, identity, recovery and translation evidence", async ({ page }) => {
  ...
  const canonicalSection = page.getByRole("region", { name: "Editable fields" });
  ...
  await expect(summaryTab).toHaveCSS("color", "rgb(37, 99, 235)");
  const summaryTranslation = page.locator('[data-translation-field="summary"]');
  await expect(summaryTranslation.getByRole("button", { name: "Retranslate" })).toHaveCount(1);
  await expect(summaryTranslation.getByText("Draft matches source.")).toBeVisible();
  await expect(page.locator('input[aria-label="Translation 7"]')).toBeVisible();
  await descriptionTab.click();
  await expect(descriptionTab).toHaveAttribute("aria-selected", "true");
  const descriptionTranslation = page.locator('[data-translation-field="description"]');
  await expect(descriptionTranslation.getByRole("button", { name: "Retranslate" })).toHaveCount(1);
  await expect(descriptionTranslation.getByRole("button", { name: "Approve + save" })).toHaveCount(1);
  await expect(descriptionTranslation.getByRole("button", { name: "Reject" })).toHaveCount(1);
  await descriptionTranslation.getByText("View source").click();
  await expect(descriptionTranslation.locator(".translation-source-markdown .wmde-markdown")).toContainText("Second line");
  await expect(descriptionTranslation.locator(".translation-source-markdown .wmde-markdown p").first()).toBeVisible();
  ...
  await expect(canonicalSection.getByLabel("Jira account ID")).toHaveValue("");
  for (const field of ["Summary", "Description", "Issue type", "Priority", "Status", "Assignee", "Due date"]) await expect(canonicalSection.getByLabel(field)).toBeVisible();
  const markdownMode = canonicalSection.getByRole("button", { name: /Switch Markdown mode/ });
  await expect(markdownMode).toHaveText("Preview");
  await markdownMode.click();
  await expect(markdownMode).toHaveText("Edit");
  ...
  await canonicalSection.getByLabel("Summary").fill("Changed summary");
  ...
  await expect(canonicalSection.getByLabel("Summary")).toHaveValue("Original");
  ...
  await descriptionTranslation.getByRole("button", { name: "Retranslate" }).click();
  await expect(page.getByText("Retranslate queued (translation-item-job); review Translation Queue.")).toBeVisible();
  const description = canonicalSection.getByLabel("Description");
  await description.fill("API đến cơ sở dữ liệu Kyoritsu");
  await description.selectText();
  await canonicalSection.getByRole("button", { name: /Add bold text/ }).click();
  await expect(description).toHaveValue("**API đến cơ sở dữ liệu Kyoritsu**");
  await description.fill("**API đến cơ sở dữ liệu Kyoritsu**");
  await canonicalSection.getByRole("button", { name: /Switch Markdown mode/ }).click();
  await expect(page.locator(".wmde-markdown strong")).toHaveText("API đến cơ sở dữ liệu Kyoritsu");
