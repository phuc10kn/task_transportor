...
- `translation_ai_provider`: ví dụ `deepseek`, `openai`.
- `translation_ai_transport`: ví dụ `openai_compatible`, `anthropic_compatible`.
- `translation_ai_model`: ví dụ `deepseek-v4-flash`, `deepseek-v4-pro`, `deepseek-chat`, `gpt-4.1-mini`, `gpt-5.4-mini`, `gpt-5.6-luna`, `gpt-5.6-terra`, `gpt-5.6-sol`.
...
OpenAI là provider tùy chọn theo Project với `translation_ai_transport = "openai_compatible"`; model mặc định là `gpt-4.1-mini`, ngoài ra có `gpt-5.4-mini`, `gpt-5.6-luna`, `gpt-5.6-terra`, `gpt-5.6-sol`; credential server-side là `OPENAI_API_KEY`. Request OpenAI không gửi field `thinking` của DeepSeek. Default toàn hệ thống vẫn là DeepSeek cho tới khi có quyết định thay đổi riêng.

Ranh giới AI:

- `src/infrastructure/external/transports/` chứa client/transport thật sự gọi ra ngoài: HTTP, OpenAI-compatible và Anthropic-compatible; provider gateway nằm ở `src/infrastructure/external/providers/`.
- Module Translation không được tự gọi `fetch`, `child_process`, `spawn`, hoặc tự biết HTTP/protocol mechanics.
- Module Translation chỉ được dùng adapter nội bộ trung tính như `TranslationAdapter`.
- Không tạo class nội bộ Translation tên `DeepSeekTranslationProvider` hoặc `OpenAiTranslationProvider`.
...
rg -n "TranslationProvider|DeepSeekTranslation|OpenAiTranslation|providerFor" src\modules\Translation src\infrastructure\external -g "*.js"
